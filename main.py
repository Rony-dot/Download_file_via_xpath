...
def process_download(url:str, xpath:str):
    try:
        ...
        # resp = session.get("http://blog.apify.com/python-lxml-tutorial/")
        ...
        # xpath_article = "//a[contains(@href,'mkv')]/@href"

        tree = etree.HTML(resp.text)
        elements = []
        for element in tree.xpath(xpath_article):
            # print(f"element: {element}")
            elements.append(f"http://172.16.50.14/{element}")
